...
fig1v=figure('Position',[-1759         245         574         412],'Color','White');Name='VPM example';
...
trace_name={'trig','whisker angle'};
fig1p=figure('Position',[-1759         245         574         412],'Color','White');Name='POm example';
plot_single_cell_example(trigs_e,trig_names,psth_e,DiscreteData(cell_example).Spikes,trace_e,sem_trace_e,trace_name,ppms,psth_bins,Name);
...
tiledlayout(2,3);
nexttile(1,[2 2]);
polarscatter(PhaseMod.pref_phase(vpmra),PhaseMod.MD(vpmra),'r.');hold on
polarscatter(PhaseMod.pref_phase(pomra),PhaseMod.MD(pomra),'b.');hold off
title('phase modulation depth','preferred phase');
nexttile(3);
histogram(PhaseMod.MD(vpmra),0:.2:max(PhaseMod.MD(cat(1,vpmra,pomra)))+.2,'FaceColor','r');hold on;box off
title('phase modulation depth');
nexttile(6);
histogram(PhaseMod.MD(pomra),0:.2:max(PhaseMod.MD(cat(1,vpmra,pomra)))+.2,'FaceColor','b');hold on;box off
...
for v=1:numel(rates_to_plot)
    nexttile(v);
    histogram(neuron_correlations_with_params{v}(vpmra,1),-1:.1:1,'FaceColor','r');
    title(['VPM corr. with ' name_vars{v}]);box off;
    nexttile(v+numel(rates_to_plot)+2);
    histogram(neuron_correlations_with_params{v}(pomra,1),-1:.1:1,'FaceColor','b');
